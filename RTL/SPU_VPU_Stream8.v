...
module SPU_VPU_Stream8 #(
    ...
    wire [31:0] lane0_index = scale_index_r[lane0_sel];
    wire [31:0] lane1_index = scale_index_r[lane1_sel];
    wire [31:0] p3_bank_base = bank_r ? P3_BANK_WORD_DEPTH : 32'd0;
    wire [31:0] lane0_word_index = p3_r ?
        (p3_bank_base + (lane0_index >> 3)) : (lane0_index >> 2);
    wire [31:0] lane1_word_index = p3_r ?
        (p3_bank_base + (lane1_index >> 3)) : (lane1_index >> 2);
    wire [2:0] lane0_scale_lane = p3_r ? lane0_index[2:0] : {1'b0,lane0_index[1:0]};
    wire [2:0] lane1_scale_lane = p3_r ? lane1_index[2:0] : {1'b0,lane1_index[1:0]};
    ...
    always @(posedge clk) begin
        if (!resetn || soft_reset) begin
            ...
            for (i = 0; i < 8; i = i + 1) begin
                raw_r[i] <= 32'sd0; row_r[i] <= 16'd0; scale_index_r[i] <= 32'd0;
                    ...
                    if (vpu_fire) begin
                        if (!bundle_index_ok) begin
                            ...
                            for (i = 0; i < 8; i = i + 1) begin
                                ...
                                scale_index_r[i] <= vpu_lane_scale_index[32*i +: 32];
                            end
