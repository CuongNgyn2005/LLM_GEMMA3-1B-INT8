...
module SPU_VPU_Stream8 #(
    ...
    // Every accepted P2 token occupies one FIFO entry.  Invalid bundles are
    // stored with an empty lane mask and retire as no-ops.  This keeps the
    // wide combinational bounds check out of every FIFO register clock-enable
    // while preserving drop/error accounting and preventing memory access.
    wire fifo_push = !split_scale_enable && vpu_fire;
    ...
    always @(posedge clk) begin
        if (!resetn || soft_reset) begin
            ...
            // P3 is direct (no FIFO), but bulk input registers still capture
            // on the simple handshake.  Validation controls only the narrow
            // request/FSM path below, not hundreds of data-register enables.
            if (split_scale_enable && vpu_fire) begin
                lane_valid_r <= bundle_index_ok ? vpu_lane_valid : 8'd0;
                for (i = 0; i < 8; i = i + 1) begin
            ...
            case (state_r)
                ...
                S_WAIT: begin
                    // Isolate the wide lane-valid completion reduction from
                    // bundle retirement, FIFO backpressure, and GEMV ready.
                    // S_RETIRE performs those updates one cycle later using
                    // only registered state as its control condition.
                    if (all_accum_done)
                        state_r <= S_RETIRE;
                end
