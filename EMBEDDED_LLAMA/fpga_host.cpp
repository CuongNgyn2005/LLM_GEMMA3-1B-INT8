...
#define FPGA_HOST_TRACE_VERSION "zcu104-gemma3-q8-v85-token-pingpong-timing"
...
// FPGA_TOKEN_TIMING=1 emits one aggregate line at each graph-sequence/token
// boundary. FPGA_PINGPONG_TIMING=1 additionally emits per-bank/per-tile lines.
...
static int g_p2_pack_workers_requested = 1;
static int g_p2_pack_workers_active = 1;
...
{
    return (g_p2_event_trace_enabled || g_token_timing_enabled || g_pingpong_timing_enabled) ? monotonic_now_us() : 0;
}
...
static void fpga_token_timing_emit(int next_graph_seq, int ubatch_tokens, const char *reason, long long end_mono_us)
{
    if (!g_token_timing_enabled || !g_token_timing.active)
    {
        return;
    }
    if (end_mono_us <= 0)
    ...
    fpga_log_line(
        true, "TOKEN_TIMING", true,
        "graph_seq=%d next_graph_seq=%d ubatch_tokens=%d scope=%s reason=%s matmuls=%lld vpu_runs=%lld "
        ...
        g_token_timing.graph_seq, next_graph_seq, ubatch_tokens, scope, reason ? reason : "unknown",
...
static void zdma_dump(const char *tag)
{
    ...
    LOGE(
        ...
        g_dma ? (unsigned long long)zdma_read_addr(&g_dma->ZDMA_CH_SRC_CUR_PYLD_LSB, &g_dma->ZDMA_CH_SRC_CUR_PYLD_MSB) : 0ULL,
        g_dma ? (unsigned long long)zdma_read_addr(&g_dma->ZDMA_CH_DST_CUR_PYLD_LSB, &g_dma->ZDMA_CH_DST_CUR_PYLD_MSB) : 0ULL,
...
static bool fpga_set_split_scale_mode(uint32_t requested_mode, const char *context)
{
    ...
    LOGPROOF("P3_MODE_TRANSITION_PASS context=%s mode=%u stream_status=0x%08x p3_status=0x%08x",
...
static bool fpga_p3_split_scale_host_self_test(void)
{
    ...
    LOGPROOF("P3_HOST_SELFTEST_PASS dense_fp16_words=8lanes max_entries=%zu max_weight_words=%zu", max_entries,
             max_words);
    return true;
}
                                               ...
                                               size_t *written_words)
{
    ...
    for (size_t pair = pair_begin; pair < pair_end; ++pair)
    {
        ...
        for (int gb = 0; gb < group_blocks; ++gb)
        {
            ...
            const block_q8_0_t *const odd_wb = odd_row < rows ? weight_block_from_base(src0, weight_data_base, row0 + odd_row,
                                                                                       k_block0 + gb)
                                                              : nullptr;
                                                       ...
                                                       int group_beats)
{
    ...
    for (size_t i = 0; i < qs_bytes; ++i)
        dst_qs[i] = expected_qs[i];
    for (size_t i = 0; i < scale_bytes; ++i)
        dst_scales[i] = expected_scales[i];
                                     ...
                                     fpga_stage_totals_t *totals)
{
    ...
    const uint32_t residency_slot =
        g_p2_weight_residency_enabled ? fpga_p2_residency_select_or_build(src0, weight_data_base, row0, rows, k_block0, group_blocks, group_beats) : P2_WEIGHT_RESIDENCY_NO_SLOT;
                                            ...
                                            int attempt)
{
    ...
    if (g_pingpong_timing_enabled)
    {
        fpga_log_line(
            ...
            job.event_vpu_done_us >= job.event_launch_us ? (double)(job.event_vpu_done_us - job.event_launch_us) / 1000.0 : 0.0,
            job.event_spu_finality_us >= job.event_launch_us ? (double)(job.event_spu_finality_us - job.event_launch_us) / 1000.0 : 0.0,
...
static bool fpga_accumulate_pl_scaled_q8_tile_job(const fpga_tile_job_t &job,
                                                  std::vector<float> &accum,
                                                  fpga_stage_totals_t *totals)
{
...
int fpga_init(void)
{
    ...
    g_token_timing_enabled = env_flag_enabled("FPGA_TOKEN_TIMING") || g_pingpong_timing_enabled;
    g_p1_sched_summary = {};
    fpga_token_timing_reset();
    if (g_token_timing_enabled)
    {
        LOGPROOF(
            "TOKEN_TIMING_CONFIG enabled=1 pingpong_detail=%d output=/tmp/fpga_debug.log "
            "token_boundary=fpga_advance_sequence_position host_to_ip=ACT_plus_WEIGHT_plus_SCALE "
            "token_read=SPU_OUT_DMA_plus_host_result_read",
            g_pingpong_timing_enabled ? 1 : 0);
    }
    if (g_p3_split_scale_requested && g_p2_input_preload_enabled)
    {
        fpga_fatal("FPGA_P3_SPLIT_SCALE=1 forbids FPGA_P2_INPUT_PRELOAD=1; P3 serializes both scale DMAs before launch");
    }
    const char *const p2_pack_workers_env = getenv("FPGA_P2_PACK_WORKERS");
    int p2_pack_workers_requested = 1;
    ...
    LOGPROOF(
        ...
        g_pl_scale_contract_check_limit > 0 ? "p2_contract_cpu_shadow" : (g_pingpong_scheduler_enabled ? "pingpong_production" : (g_spu_q8_scale_stream_supported ? "serialized_p2" : "disabled")),
    ...
    const char *mapping_policy = g_allow_devmem_fallback ? "diagnostic_all_resources" : (g_allow_vpu_devmem_compat ? "uio_dma_ddr+vpu_devmem_compat" : "uio_required");
    ...
    LOGPROOF(
        ...
        "result=0x%08x spu_out=0x%08x spu_param=0x%08x block_q8_0_size=%zu "
        "weight_layout=pair_interleaved_padded_v2 cache_format=%u residency_layout=%u",
        ...
        ACT_BASE, WEIGHT_BASE, RESULT_BASE, SPU_OUT_BASE, SPU_PARAM_BASE, sizeof(block_q8_0_t),
        FPGA_WEIGHT_CACHE_FORMAT_VERSION, P2_WEIGHT_RESIDENCY_LAYOUT_V2);
...
void fpga_cleanup(void)
{
    ...
    if (g_token_timing_enabled && g_token_timing.active)
    {
        fpga_token_timing_emit(g_token_timing.graph_seq, 0, "cleanup_incomplete", monotonic_now_us());
    }
    ...
    const long long q8_expected_fpga = q8_candidates >= q8_intentional_cpu + q8_unavailable_cpu ? q8_candidates - q8_intentional_cpu - q8_unavailable_cpu : -1;
    const bool q8_coverage_complete =
        q8_expected_fpga >= 0 && q8_unavailable_cpu == 0 && g_fpga_count == q8_expected_fpga;
    const char *fpga_block_gemv_mode = q8_candidates == 0 ? "not_applicable" : g_legacy_raw_cpu_bypass_count > 0 ? "cpu_quarantine"
                                                                           : q8_coverage_complete                ? "active"
                                                                                                                 : "incomplete";
    const uint64_t p2_residency_budget_bytes = g_p2_weight_residency_budget_mb > 0 ? (uint64_t)g_p2_weight_residency_budget_mb * 1024ULL * 1024ULL : 0ULL;
    ...
    const uint64_t p2_residency_remaining_bytes = p2_residency_allocated_bytes <= p2_residency_budget_bytes ? p2_residency_budget_bytes - p2_residency_allocated_bytes : 0ULL;
    const double p2_direct_weight_pack_mib_s =
        g_p2_residency_direct_weight_pack_us > 0 ? (double)g_p2_residency_direct_weight_pack_bytes * 1000000.0 /
    ...
    if (g_p3_retire_timing_enabled)
    {
        const uint64_t retire_timing_core_avg_ns =
            g_p3_retire_timing_valid_samples == 0U ? 0U : g_p3_retire_timing_core_total_ns / g_p3_retire_timing_valid_samples;
        LOGPROOF(
...
extern "C" void fpga_advance_sequence_position(int n_tokens)
{
    ...
    if (g_token_timing_enabled && g_token_timing.active && g_token_timing.graph_seq == previous_graph_seq)
    {
                                        ...
                                        int is_attention)
{
    ...
    fpga_token_timing_accumulate(totals, t1 - t0);
    ...
    const double cycles_per_run = (g_fpga_clock_mhz > 0.0 && totals.vpu_runs > 0) ? ((double)totals.ip_compute_us * g_fpga_clock_mhz / (double)totals.vpu_runs) : 0.0;
