...
module tb_VPU_Top;
    ...
    localparam integer NUM_LANES     = 32;
    ...
    localparam integer AXI_LANES     = DATA_WIDTH / 8;

    localparam [ADDR_WIDTH-1:0] S00_PHYS_BASE = 40'h00A0_0000_00;
    localparam [ADDR_WIDTH-1:0] S01_PHYS_BASE = 40'h00B0_0000_00;
    localparam [ADDR_WIDTH-1:0] REG_CTRL      = S00_PHYS_BASE + 40'h0000_0000;
    localparam [ADDR_WIDTH-1:0] REG_STATUS    = S00_PHYS_BASE + 40'h0000_0010;
    localparam [ADDR_WIDTH-1:0] REG_ROWS      = S00_PHYS_BASE + 40'h0000_0020;
    localparam [ADDR_WIDTH-1:0] REG_COLS      = S00_PHYS_BASE + 40'h0000_0030;
    localparam [ADDR_WIDTH-1:0] REG_COL_BEATS = S00_PHYS_BASE + 40'h0000_0040;
    localparam [ADDR_WIDTH-1:0] REG_SCALE     = S00_PHYS_BASE + 40'h0000_0050;
    localparam [ADDR_WIDTH-1:0] REG_MODE      = S00_PHYS_BASE + 40'h0000_0060;
    localparam [ADDR_WIDTH-1:0] REG_CAPS      = S00_PHYS_BASE + 40'h0000_0090;
    localparam [ADDR_WIDTH-1:0] ACT_BASE      = S00_PHYS_BASE + 40'h0001_0000;
    localparam [ADDR_WIDTH-1:0] WEIGHT_BASE   = S00_PHYS_BASE + 40'h0010_0000;
    localparam [ADDR_WIDTH-1:0] RESULT_BASE   = S00_PHYS_BASE + 40'h0020_0000;
    localparam [ADDR_WIDTH-1:0] ACT_BASE_1    = S01_PHYS_BASE + 40'h0001_0000;
    localparam [ADDR_WIDTH-1:0] WEIGHT_BASE_1 = S01_PHYS_BASE + 40'h0010_0000;
    ...
    reg  [ID_WIDTH-1:0]       awid_1;
    reg  [ADDR_WIDTH-1:0]     awaddr_1;
    reg  [7:0]                awlen_1;
    reg  [2:0]                awsize_1;
    reg  [1:0]                awburst_1;
    reg                       awlock_1;
    ...
    reg  [(DATA_WIDTH/8)-1:0] wstrb_1;
    reg                       wlast_1;
    reg                       wuser_1;
    reg                       wvalid_1;
    wire                      wready_1;

    wire [ID_WIDTH-1:0]       bid_1;
    wire [1:0]                bresp_1;
    wire                      buser_1;
    wire                      bvalid_1;
    reg                       bready_1;
    ...
    VPU_Top #(
        ...
        .S00_AXI_BASE_ADDR      (S00_PHYS_BASE),
        .S01_AXI_BASE_ADDR      (S01_PHYS_BASE)
    ) dut (
        .s00_axi_aclk       (clk),
        ...
        .s01_axi_awid       (awid_1),
        .s01_axi_awaddr     (awaddr_1),
        .s01_axi_awlen      (awlen_1),
        .s01_axi_awsize     (awsize_1),
        .s01_axi_awburst    (awburst_1),
        .s01_axi_awlock     (awlock_1),
        .s01_axi_awcache    (awcache_1),
        .s01_axi_awprot     (awprot_1),
        .s01_axi_awqos      (awqos_1),
        .s01_axi_awregion   (awregion_1),
        .s01_axi_awuser     (awuser_1),
        .s01_axi_awvalid    (awvalid_1),
        .s01_axi_awready    (awready_1),
        .s01_axi_wdata      (wdata_1),
        .s01_axi_wstrb      (wstrb_1),
        .s01_axi_wlast      (wlast_1),
        .s01_axi_wuser      (wuser_1),
        .s01_axi_wvalid     (wvalid_1),
        .s01_axi_wready     (wready_1),
        .s01_axi_bid        (bid_1),
        .s01_axi_bresp      (bresp_1),
        .s01_axi_buser      (buser_1),
        .s01_axi_bvalid     (bvalid_1),
        .s01_axi_bready     (bready_1)
    );
    ...
    function [DATA_WIDTH-1:0] pack_activation;
        ...
        begin
            ...
            for (lane = 0; lane < AXI_LANES; lane = lane + 1) begin
                idx = beat * NUM_LANES + port_sel * AXI_LANES + lane;
                if (idx < current_cols)
    ...
    task axi_write_1;
        ...
        begin
            ...
            timeout = 0;
            while (!awready_1) begin
                @(posedge clk);
                timeout = timeout + 1;
                if (timeout > 100) begin
                    fail("AXI1 write address timeout");
                    timeout = 0;
                end
            end
            @(posedge clk);
            ...
            timeout = 0;
            while (!wready_1) begin
                @(posedge clk);
                timeout = timeout + 1;
                if (timeout > 100) begin
                    fail("AXI1 write data timeout");
                    timeout = 0;
                end
            end
            @(posedge clk);
            ...
            timeout = 0;
            while (!bvalid_1) begin
                @(posedge clk);
                timeout = timeout + 1;
                if (timeout > 100) begin
                    fail("AXI1 write response timeout");
                    timeout = 0;
                end
            end

            if (bresp_1 != 2'b00)
                fail("AXI1 write response was not OKAY");
            ...
            bready_1 <= 1'b1;
            wlast_1  <= 1'b0;
            wstrb_1  <= {(DATA_WIDTH/8){1'b0}};
            wdata_1  <= {DATA_WIDTH{1'b0}};
        end
    endtask
    ...
    task run_case;
        ...
        begin
            ...
            for (beat = 0; beat < current_col_beats; beat = beat + 1) begin
                axi_write(ACT_BASE + beat * 16, pack_activation(beat, 0), 16'hffff);
                axi_write_1(ACT_BASE_1 + beat * 16, pack_activation(beat, 1), 16'hffff);
            end

            for (row = 0; row < rows; row = row + 1) begin
                for (beat = 0; beat < current_col_beats; beat = beat + 1) begin
                    axi_write(WEIGHT_BASE + ((row * MAX_COL_BEATS) + beat) * 16,
                              pack_weight(row, beat, 0), 16'hffff);
                    axi_write_1(WEIGHT_BASE_1 + ((row * MAX_COL_BEATS) + beat) * 16,
                                pack_weight(row, beat, 1), 16'hffff);
                end
            end
    ...
    task run_group_case;
        ...
        begin
            ...
            axi_write(REG_COL_BEATS, word32(group_blocks), 16'h000f);
            axi_write(REG_SCALE, word32(32'h0000_3c00), 16'h000f);
    ...
    initial begin
        ...
        awid_1 = 0; awaddr_1 = 0; awlen_1 = 0; awsize_1 = 0; awburst_1 = 0; awlock_1 = 0;
        awcache_1 = 0; awprot_1 = 0; awqos_1 = 0; awregion_1 = 0; awuser_1 = 0; awvalid_1 = 0;
        wdata_1 = 0; wstrb_1 = 0; wlast_1 = 0; wuser_1 = 0; wvalid_1 = 0; bready_1 = 1;
