...
module AXI4_Mapping #(
    ...
    function [AXI_ADDR_WIDTH-1:0] to_local_addr_from_base;
        input [AXI_ADDR_WIDTH-1:0] addr;
        input [AXI_ADDR_WIDTH-1:0] base_addr;
        begin
            if ((ENABLE_BASE_TRANSLATION != 0) && (addr >= base_addr))
                to_local_addr_from_base = addr - base_addr;
            else
                to_local_addr_from_base = addr;
        end
    endfunction

    function [31:0] local32_from_base;
        input [AXI_ADDR_WIDTH-1:0] addr;
        input [AXI_ADDR_WIDTH-1:0] base_addr;
        reg [AXI_ADDR_WIDTH-1:0] local_addr;
        begin
            local_addr = to_local_addr_from_base(addr, base_addr);
            local32_from_base = local_addr[31:0];
        end
    endfunction
    ...
    wire [31:0] wr_addr_local = local32_from_base(map_wr_addr, VPU_BASE_ADDR);
    wire [31:0] wr_addr_local_1 = local32_from_base(map_wr_addr_1, S01_BASE_ADDR);
    wire [31:0] rd_addr_local = local32_from_base(map_rd_addr, VPU_BASE_ADDR);
    ...
    wire core_wr_hit_1 =
        map_wr_en_1 && is_aux_write_addr(wr_addr_local_1) &&
        mem_index_in_range(wr_addr_local_1);
    ...
    reg core_wr_en_1_r;
    reg [1:0] core_wr_region_1_r;
    reg [31:0] core_wr_index_1_r;
    reg [AXI_DATA_WIDTH-1:0] core_wr_data_1_r;
    reg [(AXI_DATA_WIDTH/8)-1:0] core_wr_strb_1_r;
    ...
    always @(posedge clk) begin
        if (!resetn) begin
            ...
            core_wr_en_1_r    <= 1'b0;
            core_wr_region_1_r <= REGION_ACT;
            core_wr_index_1_r <= 32'd0;
            core_wr_data_1_r  <= {AXI_DATA_WIDTH{1'b0}};
            core_wr_strb_1_r  <= {(AXI_DATA_WIDTH/8){1'b0}};
        end else begin
            core_start_r      <= ctrl_start_hit;
            core_clear_done_r <= ctrl_clear_done_hit;
            core_wr_en_r      <= core_wr_hit;
            core_wr_en_1_r    <= core_wr_hit_1;
            ...
            if (core_wr_hit_1) begin
                core_wr_region_1_r <= mem_region(wr_addr_local_1);
                core_wr_index_1_r  <= mem_index(wr_addr_local_1);
                core_wr_data_1_r   <= map_wr_data_1;
                core_wr_strb_1_r   <= map_wr_strb_1;
            end
    ...
    Matrix_Vector_Multiplication #(
        ...
        .mm_wr_en_1        (core_wr_en_1_r),
        .mm_wr_region_1    (core_wr_region_1_r),
        .mm_wr_index_1     (core_wr_index_1_r),
        .mm_wr_data_1      (core_wr_data_1_r),
        .mm_wr_strb_1      (core_wr_strb_1_r),
        .mm_rd_en          (core_rd_en),
