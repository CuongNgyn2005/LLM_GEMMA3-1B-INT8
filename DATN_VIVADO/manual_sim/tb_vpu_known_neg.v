...
module tb_vpu_known_neg;
    ...
    initial begin
        ...
        $display("[VPU_KNOWN_NEG] result=%0d hex=0x%08h last=%0d expected=-64",
                 $signed(result_data), result_data, result_last);
        if (result_data !== -32'sd64)
            $display("[VPU_KNOWN_NEG] FAIL");
        else
            $display("[VPU_KNOWN_NEG] PASS");
        $finish;
    end
endmodule
