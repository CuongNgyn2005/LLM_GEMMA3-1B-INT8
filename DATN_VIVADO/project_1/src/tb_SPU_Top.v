...
module tb_SPU_Top;
    ...
    task run_scale_accum_fractional_product_case;
        reg [DATA_WIDTH-1:0] out_word;
        reg signed [63:0] got_accum;
        begin
            $display("[TB] Q8 scale-accumulate fractional product test");
            // 3.5 * 3.5 exercises non-zero high bits in p00/p01/p10 and a
            // carry from the cross-sum into the upper product word.  The
            // exact Q16.16 value for raw=1 is 12.25*65536 = 802816.
            mm_write(REGION_IN, 0,
                     pack_scale_entry(32'sd1, 16'h4300, 16'h4300,
                                      16'd2, 1'b1, 1'b1));
            start_and_wait(SPU_MODE_Q8_SCALE_ACCUM, 32'd1);
            mm_read(REGION_OUT, 0, out_word);
            got_accum = out_word[79:16];
            if (out_word[15:0] !== 16'd2 || got_accum !== 64'sd802816)
                fail("Q8 fractional scale product mismatch");
            else
                pass_count = pass_count + 1;
        end
    endtask
    ...
    initial begin
        ...
        run_scale_accum_fractional_product_case();
        run_marker_mode(SPU_MODE_SILU_MUL, "SiLU legacy engine removed");
        run_marker_mode(SPU_MODE_RMSNORM, "RMSNorm legacy engine removed");
        run_marker_mode(SPU_MODE_ROPE, "RoPE legacy engine removed");
        run_marker_mode(SPU_MODE_SOFTMAX, "Softmax legacy engine removed");
